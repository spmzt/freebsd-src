...
ATF_TC_BODY(test_rtnl_gre, tc)
{
	...
	snl_add_msg_attr_u32(&nw, IFLA_GRE_ENCAP_TYPE, IFLA_TUNNEL_GUE);
	...
	ATF_CHECK_INTEQ(attrs.ifla_encap_type, IFLA_TUNNEL_GUE);
	...
	ATF_REQUIRE((hdr = snl_finalize_msg(&nw)) != NULL);
	ATF_REQUIRE(snl_send_message(&ss, hdr));
	ATF_CHECK_INTEQ(hdr->nlmsg_type, RTM_DELLINK);
}
